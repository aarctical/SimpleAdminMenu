...
local defaultStyle = {
	...
	titleVisible = true,
	titleColor = { 254, 254, 254, 255 },
	titleBackgroundColor = { 44, 108, 188, 255 },
	titleBackgroundSprite = nil,
	subTitleColor = { 254, 254, 254, 255 },
	textColor = { 254, 254, 254, 255 },
	subTextColor = { 254, 254, 254, 255 },
...
----------
-- WarMenu.SetInstructionalButton
local function ButtonMessage(scaleform, text)
    AddTextComponentSubstringPlayerName(text)
    PushScaleformMovieFunction(scaleform, 'ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME')
...
local function setupScaleform(scaleform, data)
	...
	for n, btn in next, data do
		...
		ButtonMessage(scaleform, btn.name)
		PopScaleformMovieFunctionVoid()
	end
...
local function SetInstructions()
    form = setupScaleform('instructional_buttons', entries)
end

function WarMenu.SetInstructionalButton(name, control, enabled)
    local found = false
...
function WarMenu.ShowMenu(id, status)
	if id and menus[id] then
		if status == true then
			PlaySoundFrontend(-1, 'SELECT', 'HUD_FRONTEND_DEFAULT_SOUNDSET', true)
			setMenuVisible(id, true, true)
			WarMenu.SetInstructionalButton('Select', 191, true)
			WarMenu.SetInstructionalButton('Back', 194, true)
		elseif status == false then
			PlaySoundFrontend(-1, 'QUIT', 'HUD_FRONTEND_DEFAULT_SOUNDSET', true)
			setMenuVisible(id, false)
			WarMenu.SetInstructionalButton('Select', 191, false)
			WarMenu.SetInstructionalButton('Back', 194, false)
		...
		if skipInputNextFrame then
			...
			elseif isBackPressed() then
				if menus[currentMenu.parentId] then
					...
					WarMenu.ShowMenu(currentMenu.id, false)
				end
			end
		end
	end
